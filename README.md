...
### 기술적
프로젝트를 통해 Springboot로 Restful api를 개발하는 경험을 쌓았습니다. MVC아키텍처를 적용하여 기능을 모듈화 하고, 계층을 분리하여 설계하는 능력을 키울 수 있었습니다.

### API
서드파티 API(Strava)를 활용하여 데이터를 가져와서 처리하는 경험을 통해 직접 DB에 데이터를 저장하지 않으면서 데이터를 처리, 가공하는 기술을 배울 수 있었습니다.

### 데이터베이스 관리
MySQL을 사용하여 테이블 스키마와 쿼리문을 만들어 엔티티의 관계를 설정하고 각 엔티티에 필요한 속성들과 쿼리를 최적화 하였습니다. 

### 사용자 경험(UI/UX) 개선
프론트엔드에서 Vue.js 및 Bootstrap을 활용하여 사용자 인터페이스를 개선하는 경험을 통해 사용자 중심의 디자인 및 사용자 경험 개선에 대한 이해를 높일 수 있었습니다.

### 팀 협업 및 프로젝트 관리
백엔드파트와 프론트엔드파트 개발자를 나누어 프로젝트를 진행하기 위해 모듈화, 계층 간 분리, 디렉토리 설정 등에서는 본인의 파트가 아닌 부분도 함께 의논하는 것이 중요함을 배웠습니다. 또한 우선순위를 두고 기능 별로 문제를 해결해 나가기 위해 소통하는 능력을 키울 수 있었습니다.

### 버전 관리 및 협업 도구 사용
Github을 사용하여 협업하고 버전을 관리하는 경험을 통해 협업 시 github을 활용하는 능력을 기를 수 있었습니다. 
